...
const RES = {
    ...
    'kim': {
        family: '김', name: "생존형 락스타 '김우일'", emoji: "👑", bg: "#2c3e50",
        tags: ["#내가..", "#락앤롤", "#T1분석"],
...
function showResult(forcedKey = null, forceHorror = false, isSpecial = false) {
    ...
    const medalGradients = [
        'linear-gradient(90deg, #ffd700, #fff5b1)', // 금
        'linear-gradient(90deg, #c0c0c0, #f0f0f0)', // 은
        'linear-gradient(90deg, #cd7f32, #ffbc84)'  // 동
    ];
    ...
    for (let i = 0; i < 3; i++) {
        ...
        let displayName = `${charInfo.emoji} ${charInfo.family}씨`;
        let rankEmoji = medals[i];

        statsHtml += `
            <div class="stat-row">
                <div class="stat-name" style="width: 90px; font-size: 0.85rem;">${rankEmoji} ${displayName}</div>
                <div class="stat-bar-bg">
                    <div class="stat-bar-fill" style="width: ${percent}%; background: ${medalGradients[i]}; border: 1px solid rgba(255,255,255,0.4); box-shadow: 0 0 10px ${medalGradients[i].split(',')[1].trim()}55;"></div>
                </div>
                <div class="stat-score" style="color: #fff; opacity: 0.9;">${score}</div>
            </div>
        `;
    }
